...
async def _do_export(message_obj, telegram_id: int, user_lang: str, export_format: str, category_filter: str):
    ...
    if not export_format or export_format == 'prompt':
        keyboard = [
            ...
            [
                InlineKeyboardButton("📑 OPML (.opml)", callback_data=f"do_export_opml_{category_filter or 'all'}")
            ],
            [
    ...
    if export_format == 'html':
        ...
        lines = [
            ...
            "<H1>Bookmarks</H1>",
            "<DL><p>",
            f'    <DT><H3 ADD_DATE="{int(datetime.now(timezone.utc).timestamp())}">LensAI Saved Articles</H3>',
            "    <DL><p>"
        ]
        ...
        lines = [
            ...
            f'    <dateCreated>{datetime.now(timezone.utc).strftime("%a, %d %b %Y %H:%M:%S UTC")}</dateCreated>',
            '  </head>',
            '  <body>'
        ]
        ...
        document = io.BytesIO(("\n".join(lines)).encode('utf-8'))
        document.name = f"lensai_saved_articles{filename_category}_{timestamp}.opml"
...
async def summarize_url_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Manually follow redirects to validate each URL against SSRF
        redirects = 0
        current_url = url
        response = None
