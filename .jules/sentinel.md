...
## 2024-06-25 - [HIGH] Prevent CSV/Formula Injection during data export
**Vulnerability:** The data export function `_clean_export_value` in `functions/telegram_bot.py` wrote user-controlled data directly to CSV cells without escaping prefix characters that spreadsheet applications (like Microsoft Excel, Google Sheets) interpret as formulas (e.g., `=`, `+`, `-`, `@`). This allowed an attacker to input a malicious payload (e.g., `=cmd|' /C calc'!A0`) as their username, URL, or content title, leading to arbitrary command execution when an administrator or user exported and opened the file.
**Learning:** Spreadsheet programs evaluate cells starting with certain special characters as formulas rather than raw text. When exporting untrusted data to CSV formats, failure to sanitize these prefixes creates a severe client-side vulnerability (CSV Injection).
**Prevention:** Always sanitize data intended for CSV or spreadsheet export by checking if the value starts with `=`, `+`, `-`, or `@`. If it does, safely prepend it with a single quote (`'`), which instructs the spreadsheet engine to treat the contents as literal text rather than an executable formula.
