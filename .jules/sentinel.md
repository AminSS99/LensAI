...
## 2024-05-24 - [MEDIUM] Fix Timing attack vulnerability in secret validation
**Vulnerability:** In `functions/main.py`, the secret validation for `telegram_webhook` and `_require_internal_secret` was using simple string equality (`!=`) instead of a constant-time comparison, making it susceptible to timing attacks.
**Learning:** Standard string comparisons can exit early, taking an amount of time that depends on the matched prefix length. Attackers can exploit this to discover secrets character by character.
**Prevention:** Always use constant-time comparison functions like `hmac.compare_digest` for validating cryptographic secrets or tokens. Also, provide a fallback (like an empty string `""`) when retrieving headers to avoid `TypeError` when a header is completely missing.
