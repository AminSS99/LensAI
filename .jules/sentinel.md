...
## 2026-04-28 - [HIGH] Prevent CSV Formula Injection during export
**Vulnerability:** When users exported their saved articles to an Excel CSV format in `functions/telegram_bot.py`, the fields (such as title or source) were not sanitized against starting with special characters like `=`, `+`, `-`, or `@`. An attacker could save an article with a malicious title (e.g., `=cmd|' /C calc'!A0`) and when another user or the same user exported the list to CSV and opened it in Excel, the malicious formula would execute (CSV Injection / Formula Injection).
**Learning:** Exporting user-controlled data to CSV format without sanitization is a security risk if the file is opened by spreadsheet applications (like Excel) which execute formulas. Simple string cleaning is not enough.
**Prevention:** To prevent CSV Injection (Formula Injection), always check if the exported string starts with dangerous formula prefixes (`=`, `+`, `-`, `@`) and prepend them with a single quote (`'`). This tells the spreadsheet software to treat the contents as a literal string rather than an executable formula.
