...
## 2025-04-15 - [HIGH] Secure Telegram Webhook Endpoint
**Vulnerability:** The Telegram webhook endpoint in `functions/main.py` was publicly accessible without any authentication or authorization. An attacker could potentially send arbitrary payloads directly to the webhook, bypassing Telegram, to exploit internal logic or cause denial of service.
**Learning:** Webhooks that trigger sensitive business logic or external actions must authenticate the origin of requests to ensure they actually come from the expected service.
**Prevention:** Utilize the `X-Telegram-Bot-Api-Secret-Token` header mechanism provided by Telegram to validate the authenticity of incoming requests by comparing it against a securely stored `TELEGRAM_WEBHOOK_SECRET_TOKEN`.
