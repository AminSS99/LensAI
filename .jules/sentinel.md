...
## 2026-05-15 - [HIGH] Prevent HTTP Parameter Pollution / SSRF via URL encoding
**Vulnerability:** In `functions/deep_dive.py`, the `find_hn_discussion` function constructed the `api_url` using f-string interpolation: `f"https://hn.algolia.com/api/v1/search?query={url}&tags=story&hitsPerPage=3"`. This allows an attacker to inject arbitrary query parameters (like `&tags=somethingelse`) by including them in the `url` parameter, potentially altering the backend API request.
**Learning:** Directly interpolating user input into URL query strings is dangerous because special characters are not safely encoded, leading to HTTP Parameter Pollution and potentially SSRF in backend requests.
**Prevention:** Always use the `params` dictionary argument provided by HTTP clients (e.g., `client.get(api_url, params=params)`) instead of string interpolation for query parameters. This ensures that the HTTP client library safely and correctly URL-encodes all user input.
