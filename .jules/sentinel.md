...
## 2024-05-24 - [CRITICAL] Prevent unauthorized webhook access
**Vulnerability:** The Telegram webhook handler in `functions/main.py` did not validate the `X-Telegram-Bot-Api-Secret-Token` header. This allowed any unauthenticated third party to send forged requests to the bot, potentially executing commands or sending arbitrary messages.
**Learning:** Webhook endpoints that accept data and trigger actions must enforce strong authentication to verify the source of the requests. In Telegram's case, failing to check the secret token exposes the application to severe spoofing risks.
**Prevention:** Always validate the `X-Telegram-Bot-Api-Secret-Token` request header against a secure, randomly generated environment variable (e.g., `TELEGRAM_WEBHOOK_SECRET_TOKEN`) and immediately return `403 Forbidden` if they do not match.
