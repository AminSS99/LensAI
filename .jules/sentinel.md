...
## 2024-06-25 - [HIGH] Prevent CSV Injection (Formula Injection) in Data Exports
**Vulnerability:** The `_do_export` function in `functions/telegram_bot.py` exported user-controlled data (like `title`, `source`, and `category`) directly into a CSV file without sanitization. An attacker could craft a malicious article title starting with `=`, `+`, `-`, or `@`, causing the resulting CSV to execute arbitrary commands or formulas when opened by a victim using spreadsheet software like Microsoft Excel.
**Learning:** Exporting raw user input to formats like CSV or Excel can lead to arbitrary code execution or data exfiltration via Formula Injection if the input starts with specific formula trigger characters.
**Prevention:** To prevent CSV Injection, sanitize all fields written to CSV by prepending a single quote (`'`) to any value that starts with `=`, `+`, `-`, or `@`. This forces the spreadsheet software to treat the contents as a literal string rather than an executable formula.
