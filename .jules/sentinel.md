...
## 2025-02-18 - [CRITICAL] Fix missing authentication on webhook endpoint
**Vulnerability:** The Telegram webhook endpoint `telegram_webhook` in `functions/main.py` did not validate incoming requests. This meant anyone who knew or discovered the Cloud Function URL could send spoofed Telegram updates to the bot.
**Learning:** Cloud Functions acting as webhooks need their own layer of authentication to verify the sender. Telegram provides a feature (`X-Telegram-Bot-Api-Secret-Token`) for this exact purpose, but it must be explicitly checked. Regular string comparison is vulnerable to timing attacks.
**Prevention:** Always validate webhook requests. For Telegram, check the `X-Telegram-Bot-Api-Secret-Token` header against a stored secret using a constant-time comparison function like `hmac.compare_digest` to prevent timing attacks.
