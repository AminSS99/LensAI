...
## 2024-04-24 - [HIGH] Prevent Webhook Authorization Bypass via Timing Attacks
**Vulnerability:** The Telegram webhook in `functions/main.py` lacked robust verification of the `X-Telegram-Bot-Api-Secret-Token` header. Comparing secret tokens using standard string equality operators (like `!=` or `==`) is susceptible to timing attacks, allowing an attacker to deduce the secret byte-by-byte based on the time it takes the comparison to fail.
**Learning:** Security-sensitive string comparisons, especially for authentication or authorization tokens, must always use constant-time comparison functions to prevent timing side-channel attacks.
**Prevention:** Use `hmac.compare_digest(provided_token, expected_token)` instead of `==` or `!=` for comparing cryptographic hashes or secret tokens.
