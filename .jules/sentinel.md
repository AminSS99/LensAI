...
## 2024-06-25 - [MEDIUM] Fix TypeError DoS in JSON Export
**Vulnerability:** The JSON export endpoint in `functions/telegram_bot.py` attempted to serialize user articles containing `datetime` objects using `json.dumps()` without providing a custom encoder. This resulted in a `TypeError: Object of type datetime is not JSON serializable`, causing the application to crash and preventing the user from exporting their data.
**Learning:** Standard JSON encoding libraries often fail natively on complex data types like `datetime` or custom objects. If user-generated or database-returned data isn't carefully sanitized or properly encoded, an unhandled exception during serialization can lead to localized application failure or denial of service.
**Prevention:** Always use a custom JSON encoder (e.g., subclassing `json.JSONEncoder`) that explicitly handles fallback types like `datetime` (using `.isoformat()`) when serializing potentially complex object hierarchies from databases.
